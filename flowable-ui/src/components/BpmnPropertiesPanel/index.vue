  ...
  <div class="process-panel__container" :style="{ width: `${this.width}px` }">
    <el-collapse v-model="activeTab">
      ...
      <el-collapse-item v-if="elementType.indexOf('Task') !== -1" key="task" name="task">
        ...
        <element-task :id="elementId" :type="elementType" :user-data="userData" />
      </el-collapse-item>
...
export default {
  ...
  mounted() {
    User.list({ listMode: true }).then(ret => {
      ...
      // this.userDataMap = transToMap(ret.data.records, 'userName', 'nickName')
    })
  },
  created() {
    this.initModels()
  ...
  methods: {
    ...
    initFormOnChanged(element) {
      ...
      if (!activatedElement) {
        activatedElement = window.bpmnInstances.elementRegistry.find(el => el.type === 'bpmn:Collaboration')
      }
