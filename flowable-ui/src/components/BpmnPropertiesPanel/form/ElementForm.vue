...
export default {
  ...
  methods: {
    resetFormList() {
      ...
      // 获取元素扩展属性 或者 创建扩展属性
      var elExtensionElements = this.bpmnElement.businessObject.get('extensionElements') || window.bpmnInstances.moddle.create('bpmn:ExtensionElements', { values: [] })

      var tempValues = elExtensionElements.values.filter(ex => ex.$type === `${this.prefix}:FormData`)
      // 获取元素表单配置 或者 创建新的表单配置
      this.formData = tempValues ? tempValues[0] : window.bpmnInstances.moddle.create(`${this.prefix}:FormData`, { fields: [] })
      //  this.formData =tempValues.filter(ex => ex.$type === `${this.prefix}:FormData`)?.[0]||window.bpmnInstances.moddle.create(`${this.prefix}:FormData`, { fields: [] });

      // 业务标识 businessKey， 绑定在 formData 中
      this.businessKey = this.formData.businessKey

      // 保留剩余扩展元素，便于后面更新该元素对应属性
      this.otherExtensions = elExtensionElements.values.filter(ex => ex.$type !== `${this.prefix}:FormData`)
