...
export default {
  ...
  data() {
    return {
      ...
      fileName: '工作流的流程任务的拓展表', // 文件名
      importFunction: CustomTask.importData, // 导入方法
      templateFunction: CustomTask.exportData, // 模板方法，有自定义模板则不传
      rules: {
        assigneeUserId: [{ required: true, message: '不能为空', trigger: 'change' }],
        name: [{ required: true, message: '不能为空', trigger: 'change' }],
        taskId: [{ required: true, message: '不能为空', trigger: 'change' }],
        ...
        processInstanceId: [{ required: true, message: '不能为空', trigger: 'change' }],
        processDefinitionId: [{ required: true, message: '不能为空', trigger: 'change' }]
      }
    }
  },
  mounted() {
  ...
  methods: {
    ...
    getList() {
      ...
      CustomTask.getTodoTaskPage(this.listQuery).then(response => {
    ...
    /** 处理审批按钮 */
    handleAudit(row) {
      this.$router.push({ path: '/flowable/audit', query: { processInstanceId: row.processInstanceId, taskId: row.taskId }})
    },
    sortChange(data) {
